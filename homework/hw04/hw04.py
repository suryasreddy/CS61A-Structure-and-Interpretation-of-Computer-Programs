...
def replace_leaf(t, find_value, replace_value):
    ...
    nw = [replace_leaf(b, find_value, replace_value) for b in branches(t)]
    return tree(label(t), nw)
...
def quadratic(x, a, b, c):
    ...
    l, u, e = map(f, (upper_bound(x), lower_bound(x), extrema))
    if extrema >= lower_bound(x) and extrema <= upper_bound(x):
        return interval(min(l, e, u), max(l, e, u))
    else:
